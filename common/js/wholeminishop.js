...
$(document).ready(function(){
    ...
    $(document).on('click', '.categorySubMenuProduct', function() {
        ...
        $.ajax({
            ...
            data: {action: 'getSubCategoryProductMiniShop', cid: catProductId, wid: catWholId},
            type: 'post',
            async: false,
            beforeSend: function() {
                $('#wh_product').html('<div class="products_outer"><div class="products_box"><img src="'+SITE_ROOT_URL+'common/images/loader1.gif"/></div></div>');
    ...
    $("body").delegate('.childMenu','mouseenter mouseleave',function(e) { //alert('test');
        var cId = $(this).attr('id');
        if(e.type === 'mouseenter'){
        $.ajax({
            url: SITE_ROOT_URL+"common/ajax/ajax.php",
            data: {action: 'getSubCategoryMiniShop', cid: cId, wid: wID},
            type: 'post',
            async: false,
            success: function(data) {
                $('#' + cId).append(data);
