...
class customer_feedbackCtrl extends Paging {
    ...
    private function getList() {
        ...
        global $objGeneral;
        $varPortalFilter = $objGeneral->countryPortalFilter($_SESSION['sessAdminWholesalerIDs'],'wf.fkWholesalerID');
        ...
        $varWhereClause = "wf.fkWholesalerID = '" .$_GET['wid']."' ";
        if (isset($_REQUEST['frmSearchPressed']) && $_REQUEST['frmSearchPressed'] == 'Yes') {
            ...
            $CustomerName = $arrSearchParameter['CustomerName'];
            if ($CustomerName <> '') {
                $varWhereClause .= " AND CustomerFirstName LIKE '%" . addslashes($CustomerName) . "%'";
            }
            if ($ProductId <> '') {
                $varWhereClause .= " AND pkProductID = " . $ProductId . "";
            }
            ...
            $arrRecords = $objcustomer->productDetailsFeedback($varWhereClauses);
            ...
            $this->arrRows = $objcustomer->productDetailsFeedback($varWhereClauses, $this->varLimit);
            $this->arrPerform = $objcustomer->wholesalerPerformance($varWhereClauses);
        } else {
            $varWhereClauses = $varWhereClause. $varPortalFilter;
