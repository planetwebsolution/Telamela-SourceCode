...
if ($_POST['action'] == 'verifyEmailLogistic')
{
    $fid = $_POST['email'];
    $objCore = new Core();
    echo $objCore->verifyEmailLogistic($fid);
}
...
if($_POST['action']=='homeBannerAdvertisment1'){
    $objHome =new Home();
    $varAdds=$objHome->getAdsOne();
    
    ?>
    <a href="<?php echo $varAdds[0]['AdUrl']; ?>" target="_blank">
            <img src="<?php echo UPLOADED_FILES_URL . "images/ads/276x160/" . $varAdds[0]['ImageName']; ?>" src="" alt="<?php echo $varAdds[0]['Title']; ?>" title="<?php echo $varAdds[0]['Title']; ?>">
     </a>
   
    <?php
    
}
if($_POST['action']=='homeBannerAdvertisment2'){
    $objHome =new Home();
    $varAdds=$objHome->getAdsTwo();
...
if($_POST['action']=='homeBannerAdvertisment3'){
    $objHome =new Home();
    $varAdds=$objHome->getAdsThree();
...
if($_POST['action']=='homeBannerAdvertisment4'){
    $objHome =new Home();
    $varAdds=$objHome->getAdsFour();
...
if($_POST['action']=='homeBannerAdvertisment5'){
    $objHome =new Home();
    $varAdds=$objHome->getAdsFive();
   ?>
                <div class="banners_new"><a href="<?php echo $varAdds[0]['AdUrl']; ?>" target="_blank">
                        <img src="<?php echo UPLOADED_FILES_URL . "images/ads/276x160/" . $varAdds[0]['ImageName']; ?>" src="" alt="<?php echo $varAdds[0]['Title']; ?>" title="<?php echo $varAdds[0]['Title']; ?>">
                    </a>
                </div>
                <?php
...
if($_POST['action']=='getPackageRelatedProducts'){
    $objCore = new Core();
    $objProduct=new Product();
    $objPackageId =trim($_POST['pid']);
    $arrPackageProducts=$objProduct->getPackageProducts($objPackageId);
    
    ?>  
    <div id="contentBox" class="contentBox_<?php echo $objCatId;?>">
        
     <?php                                                                        
        if (count($arrPackageProducts) > 0)
        {?>
        <h2>
        Products
        </h2><br><br>
        <table cellspacing="0" cellpadding="4" border="1" style="width:100%" valign="middle" id="colorBox_table">
            <tbody>
                <tr class="heading">
                    <td align="center"><strong>Product Image</strong></td>
                    <td align="center" style="width:300px"><strong>Product Name</strong></td>
                </tr>
                <?php
                    foreach($arrPackageProducts as $product):
                    ?>
                    <tr>
                        <td align="center" valign="top">
                            <img src="<?php echo $objCore->getImageUrl($product['ProductImage'], 'products/85x67'); ?>" alt="<?php echo $product['ProductName'] ?>"/>
                        </td>
                        <td align="left" valign="top">
                            <span style="text-align:center"><?php echo $product['ProductName'];?></span>
                        </td>
                    </tr>   
                    <?php
                endforeach;
        ...
        {
            ?>
            <tr><td colspan="10" align="center">No Products linked!</td></tr>
        <?php } ?> 
    </div>
<?php
}
