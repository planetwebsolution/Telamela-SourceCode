...
// Calling mail function
    ...
    public function getLeftPart($argArrPOST)
    {
        /*$tableCoupon = TABLE_CATEGORY;
        $arrCouponClms = array('pkCategoryId', 'CategoryName');
        $whereCoupon = "CategoryStatus='1' AND CategoryParentId='".$argArrPOST['catId']. "'";
        ...
        $varQuery = "SELECT pkCategoryId,CategoryName FROM ".TABLE_CATEGORY." where CategoryParentId='".$argArrPOST['catId']."' AND CategoryStatus='1' ";
        $arrResProd = $this->getArrayResult($varQuery);
        ...
        // for price start here ------------------
        $priceArr = array();
        foreach($arrResProd as $key=>$val)
        {
            $varQuery2 = "SELECT FinalPrice FROM ".TABLE_PRODUCT." where fkCategoryID='".$val['pkCategoryId']."' AND ProductStatus='1' AND Quantity>0";
            $arrResPrice = $this->getArrayResult($varQuery2);
            foreach($arrResPrice as $priceKey=>$priceVal)
            {
                if(!in_array($priceVal['FinalPrice'], $priceArr))
                {
        ...
        // for attributes start here ---------------------------------------------
        
        $attrArr = array();
        foreach($arrResProd as $key=>$val)
        { 
            $varQuery2 = "SELECT fkAttributeId FROM ".TABLE_ATTRIBUTE_TO_CATEGORY." where fkCategoryID='".$val['pkCategoryId']."' ";
            $arrResAttr = $this->getArrayResult($varQuery2);
            foreach($arrResAttr as $attrKey=>$attrVal)
            {
                if(!in_array($attrVal['fkAttributeId'], $attrArr))
                {
                    array_push($attrArr, $attrVal['fkAttributeId']);
                }
            }
        }
        
        //print_r($attrArr);
        /*foreach($attrArr as $attKey=>$attVal)
        {
            $arrAttribute = $objShoppingCart->getAttributeName($argArrPOST['sCatId'], $attVal);
            $attrResp[attribute]= $arrAttribute;
        }*/
        
        //pre($attrResp);
        //$arrAttribute = $objShoppingCart->getAttributeName($argArrPOST['sCatId'], $attrArrProvided);
        //pre($attrArr);
...
pre($arrRes);*/
        $limit = $_arrPost['limit'] ? "LIMIT {$_arrPost['limit']}" : '';
        $varID = "o.fkCustomerID = '" . $_arrPost['cid'] . "' ";
        $query = "SELECT o.pkOrderID,o.TransactionID,Status as status FROM " . TABLE_ORDER . " as o INNER JOIN " . TABLE_ORDER_ITEMS . " as oi ON (o.pkOrderID = oi.fkOrderID AND {$varID}) LEFT JOIN " . TABLE_ORDER_TOTAL . " as ot ON (o.pkOrderID = ot.fkOrderID AND ot.Code='total') GROUP BY pkOrderID,Status,fkWholesalerID order by o.pkOrderID desc {$limit}";
        $arrRes = $this->getArrayResult($query);
        return $arrRes;
    }
    ...
    function getCustomerOrderDetails($_arrPost)
    {
        ...
        $arrClms = array('a.BillingFirstName,a.BillingLastName,a.BillingOrganizationName,a.BillingAddressLine1,a.BillingAddressLine2,a.BillingPostalCode', 'd.name as BillingCountryName', 'e.name as ShipingCountryName');
...
//pre($arrRes);
    ...
    function shipping($argRequest = null) {
        ...
        foreach($varCartData['Product'] as $key=>$val)
        {
            ...
            {                
                ...
                $isDom = 1;
                foreach($fkShippingID as $shopKey=>$shopVal)
                {
                    $arrData['products'][$a]['ShippingDetails'] = $this->getShippingDetailsForProduct($pDetaisl[0]['pkProductID'], $shopVal, $kv['qty'], 0, $isDom, $arrCustomer);
                }
    ...
    {        
        $varQuery = "SELECT fkShippingID FROM " . TABLE_PRODUCT . " WHERE pkProductID  = '" . $varCustomerId . "'";
        $arrCustomerDetails = $this->getArrayResult($varQuery);
        return $arrCustomerDetails[0];
    }
