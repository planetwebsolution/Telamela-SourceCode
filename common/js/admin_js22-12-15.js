...
function validateShipplingGatewaysForm(e) {  //alert('test11');return false;
    ...
    if ("" == e.frmShippingMethodID.value || "0" == e.frmShippingMethodID.value)
        return alert("Shipping method is Required!"), e.frmShippingMethodID.focus(), !1;
    if (!(t.length > 0))
        return!0;
    for (var f = 0; f < t.length; f++) {
        if ("" == t[f].value)
            return alert("Weight is Required!"), t[f].focus(), !1;
        if (t[f].value < 0)
            return alert("Weight should not be less then 1!"), t[f].focus(), !1;
        if (0 == AcceptDecimal(t[f].value))
            return alert("Please Enter numeric or decimal value!"), t[f].select(), !1;

        if ("" == r[f].value && "" == a[f].value && "" == i[f].value && "" == n[f].value && "" == l[f].value && "" == o[f].value && "" == u[f].value && "" == s[f].value && "" == m[f].value && "" == c[f].value && "" == d[f].value)
            return alert("Zone is Required!"), r[f].focus(), !1;
        if (r[f].value < 0) {
