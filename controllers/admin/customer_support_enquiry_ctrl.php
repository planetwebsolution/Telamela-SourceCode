...
//pre($varPortalFilter);
	...
	$this->varPageStart = $objPaging->getPageStartLimit($varPage, $this->varPageLimit);
        ...
        $arrRecords = $objSupport->viewAllCustomerSupport($varWhr, $limit = '');
        ...
        $this->arrRows = $objSupport->viewAllCustomerSupport($varWhr, $this->varLimit);
	...
	$this->varWholesalerList = $objSupport->wholeSalerList();
        ...
        if (isset($_POST['frmHidenAdd']) && ($_POST['frmHidenAdd'] == 'message')) {   // Editing images record
            ...
            if ($varAddStatus=='no') {
                ...
                //header('location: customer_support_compose_manage_uil.php');
                //die;
	 ...
	 if (isset($_POST['frmHidenAdd']) && ($_POST['frmHidenAdd'] == 'reply')) {   // Editing images record
            ...
            $varAddStatus = $objSupport->addReplyCustomerSupport($_POST);
            if ($varAddStatus > 0) {
                $objCore->setSuccessMsg(ADMIN_SUPPORT_ADD_SUCCUSS_MSG);
                header('location: customer_support_enquiry_manage_uil.php');
                die;
