...
class CountryPortalInvoiceCtrl extends Paging {
    /*
    ...
    private function getList() {
        ...
        if ($_SESSION['sessUserType'] == 'user-admin') {
            $varPortalFilter = " AND FromUserID='" . $_SESSION['sessUser'] . "'";
        } else {
            $varPortalFilter = "";
        }
        ...
        if (isset($_REQUEST['frmSearchPressed']) && $_REQUEST['frmSearchPressed'] == 'Yes') {
            ...
            if ($cName <> '') {
                $varWhereClause .= " AND AdminTitle LIKE '%" . addslashes($cName) . "%'";
            }

            if ($amount <> '') {
                $varWhereClause .= " AND Amount > '" . addslashes($amount) . "'";
            }
            if ($iDateFrom <> DATE_NULL_VALUE_SITE && $iDateTo <> DATE_NULL_VALUE_SITE) {
        ...
        $arrRecords = $objCountryPortal->getInvoiceList($varWhrClause);
        ...
        $this->arrRows = $objCountryPortal->getInvoiceList($varWhrClause, $this->varLimit);
    ...
    public function pageLoad() {
        ...
        if (isset($_GET['iid']) && $_GET['iid'] != '' && ($_GET['type'] == 'view')) {
            $varID = $_GET['iid'];
            $this->arrRow = $objCountryPortal->viewInvoice($varID, $varPortalFilter);
            //pre($this->arrRow);
        } else if (isset($_GET['iid']) && $_GET['iid'] != '' && ($_GET['type'] == 'delete')) {
            if ($_SESSION['sessUserType'] == 'super-admin' || in_array('delete-country-office-invoice', $_SESSION['sessAdminPerMission'])) {
               
                $varID = $_GET['iid'];
                $this->arrRow = $objCountryPortal->removeInvoice($varID, $varPortalFilter);
...
// end of page load
    ...
    function exportInvoice($arrPost) {
        ...
        $arrInvoice = $objCountryPortal->invoiceList('', '');
        //pre($arrInvoice);
        $headings = array(
            'Invoice ID',
            'Invoice Date',
...
$objPage = new CountryPortalInvoiceCtrl();
$objPage->pageLoad();
?>
