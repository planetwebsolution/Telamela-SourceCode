...
switch ($case) {
    case 'addToCart':
        ...
        if ($objShoppingCart->productInStock($_REQUEST['pid'], $qty)) {
            ...
            $_SESSION['MyCart']['Total'] = $_SESSION['MyCart']['Total'] + $qty;
...
//          pre($_SESSION['MyCart']['Total']);
    ...
    case 'RemoveProductFromCartOnBox':
        ...
        $arrRow = $objClassCommon->getProduct("ProductName", " pkProductID = '" . $_REQUEST['pid'] . "' ");
        //die((int) $_SESSION['MyCart']['Total'] . ',' . $arrRow[0]['ProductName']);
...
<li class="last"><span>Grand Total</span><strong>' . $_SESSION['SiteCurrencySign'] . ' ' . $varGrandTotal . '</strong></li>';
    ...
    case 'addToProductCart':
        ...
        if ($objShoppingCart->productInStock($_REQUEST['pid'], $_REQUEST['qty'], $_REQUEST['optIds'], $arrAttr)) {
            ...
            $_SESSION['MyCart']['Total'] = $_SESSION['MyCart']['Total'] + $_REQUEST['qty'];
...
//print_r($_SESSION);
    ...
    case 'CartPopupForSuccess':
        $objCustomer = new Customer();
        $argWhere = "pkCustomerID='" . $_SESSION['sessUserInfo']['id'] . "' ";
        $arrCustomerRewards = $objCustomer->select(TABLE_CUSTOMER, array('BalancedRewardPoints'), $argWhere);
        echo $arrCustomerRewards[0]['BalancedRewardPoints'];
        break;
