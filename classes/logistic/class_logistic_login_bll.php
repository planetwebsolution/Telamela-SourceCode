...
class AdminLogistic extends Database {
    /*     * ****************************************
    ...
    function doAdminLogin($argArrPOST) {
        ...
        if (isset($argArrPOST)) {
            ...
            $varWhereCondition = " AND (logisticEmail = binary '" . addslashes($varAdminUserName) . "') AND logisticPassword = binary ('" . addslashes($varAdminPassword) . "')";
            ...
            if ($varResult > 0) {
                ...
                $arrClmn = array('logisticportalid', 'logisticStatus');

                $arrAdmin = $this->getLogisticInfo($varWhereCondition);
                $arrsuperAdmin = $this->getsuperadminmailid();
                $superadminmailid = $arrsuperAdmin[0]['AdminEmail'];
                //pre($arrAdmin[0]['logisticportal']);
                //$arrCoumtry = $this->getCurrentCountry($arrAdmin[0]['logisticportal']);
                // = $this->getAdminWholesalers($arrAdmin[0]['pkAdminID']);
                //pre($varAdminWholesalers);
                // CHECK FOR INACTIVE MODERATORS.

                if ($arrAdmin[0]['logisticStatus'] == '0') {

                    $objCore->setErrorMsg("Your account has been deactivated. Please contact Telamela.");
                ...
                $_SESSION['sessLogistic'] = $arrAdmin[0]['logisticportalid'];

                if ($_SESSION['sessLogistic'] != 0) {
                    $_SESSION['sessLogistictype'] = 'logistic-admin';
                    $_SESSION['sessLogisticCountry'] = $arrAdmin[0]['name'];
                    //$_SESSION['sessLogisticCountryId'] = $arrAdmin[0]['country_id'];
                    $_SESSION['sessLogisticUserName'] = $varAdminUserName;

                    $_SESSION['sessLogisticPassword'] = md5($varAdminPassword);
                    $_SESSION['sessLogisticTitle'] = $arrAdmin[0]['logisticTitle'];
                    $_SESSION['sessLogisticEmail'] = $arrAdmin[0]['logisticEmail'];
                    $_SESSION['sessLogisticPortal'] = $arrAdmin[0]['logisticportal'];
                    $_SESSION['sesslogisticAdminEmail'] = $superadminmailid;
                    // $_SESSION['sessUserType']='logistic-portal';
                ...
                $_SESSION['sessLogistic'] = '';
                $_SESSION['sessLogistictype'] = '';
                // $_SESSION['sessUserType'] = '';

                $_SESSION['sessLogisticUserName'] = '';

                $_SESSION['sessLogisticPassword'] = '';

                $_SESSION['sessLogisticTitle'] = '';

                $_SESSION['sessLogisticEmail'] = '';

                $_SESSION['sessLogisticPortal'] = '';

                $_SESSION['sesslogisticAdminEmail'] = '';

                unset($_SESSION['sessLogistic']);
                // unset($_SESSION['sessUserType']);
                unset($_SESSION['sessLogistictype']);

                unset($_SESSION['sessLogisticUserName']);

                unset($_SESSION['sessLogisticPassword']);

                unset($_SESSION['sessLogisticTitle']);

                unset($_SESSION['sessLogisticEmail']);

                unset($_SESSION['sessLogisticPortal']);
                unset($_SESSION['sesslogisticAdminEmail']);
    ...
    function checkAdminSession() {
        //pre($_SESSION);
        if (($_SESSION['sessLogisticUserName'] != '') && ($_SESSION['sessLogisticPassword'] != '')) {

            $varSessAdminUserName = $_SESSION['sessLogisticUserName'];

            $varSessAdminPassword = $_SESSION['sessLogisticPassword'];


            //$varWhereCondition = " AND (logisticEmail = binary '" . addslashes($varAdminUserName) . "') AND logisticPassword = binary ('" . addslashes($varAdminPassword) . "')";
            $varWhereCondition = "logisticEmail = '" . addslashes($varSessAdminUserName) . "' AND logisticPassword = '" . addslashes($varSessAdminPassword) . "' ";

            $result = $this->getNumRows(TABLE_LOGISTICPORTAL, $varWhereCondition);
...
//pre("here");
    ...
    function getAdminNumRows($argWhrCon) {
        ...
        $varAdminClmn = 'logisticportalid';

        $varNumRows = $this->getNumRows(TABLE_LOGISTICPORTAL, $varAdminClmn, $argWhrCon);
    ...
    function changeAdminPassword($argArrPOST) {
        ...
        if ($errorMsg) {
            ...
            $varAdminID = $_SESSION['sessLogistic'];
...
//        $varPath = "<img src = ".SITE_ROOT_URL.'admin/images/logo.png'.">";
    ...
    function getLogisticInfo($argWhere) {
        ...
        $arrClms = array('logisticportalid', 'name', 'logisticUserName', 'logisticEmail', 'logisticTitle', 'logisticStatus', 'logisticportal');
        $varTable = TABLE_LOGISTICPORTAL . ' INNER JOIN ' . TABLE_COUNTRY . ' ON logisticportal = country_id ';
        $varWhere = '1 ' . $argWhere;
    ...
    function getsuperadminmailid() {

        $arrClms = array('pkAdminID', 'AdminType', 'fkAdminRollId', 'AdminUserName', 'AdminLastLoginIPAddress', 'AdminPassword', 'AdminEmail', 'PaypalEmailID', 'AdminTitle', 'AdminCountry', 'AdminRegion', 'Commission','AdminStatus', );
...
//    function getCurrentCountry($portalId) {
//        
//        $arrClms = array('AdminCountry','name');
//        $varTable = TABLE_ADMIN." LEFT JOIN " . TABLE_COUNTRY . " ON AdminCountry = country_id" ;
//        $varWhere = '1 AND  pkAdminID = '. $portalId .'' ;
//
//        $arrCountryResults = $this->select($varTable, $arrClms, $varWhere);
//        //pre($arrCountryResults);
//        return $arrCountryResults;
//    }
