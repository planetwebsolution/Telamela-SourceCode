    ...
    public function pageLoad() {
        ...
        if (isset($_POST['frmHidenAdd']) && $_POST['frmHidenAdd'] == 'add' && $_GET['type'] == 'add') {   // Editing images record
            ...
            if ($varAddStatus == 'exist') {
                $objCore->setErrorMsg(ADMIN_SHIPPING_ALREADY_EXISTS);
                ...
                header('location:shipping_manage_uil.php');
                die;
            } else {
            ...
            if ($varUpdateStatus == 'exist') {
                $objCore->setErrorMsg(ADMIN_SHIPPING_ALREADY_EXISTS);
                header('location:shipping_edit_uil.php?type=' . $_GET['type'] . '&smid=' . $_GET['smid'] . '&httpRef=' . $_POST['httpRef']);
                die;
