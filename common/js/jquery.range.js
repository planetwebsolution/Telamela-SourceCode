	...
	jRange.prototype = {
		...
		showPointerValue: function(pointer, position, animate){
			...
			label[animate?'animate':'css']({left: left});
                        $('.back-bar').find('.pointer-label:eq(1)').css('left','80%');
                        $('.back-bar').find('.pointer-label:eq(0)').css('left','-5px');
			this.setInputValue(pointer, value);
		},
		valuesToPrc: function(values){
			var lowPrc = this.interval!=0?( ( values[0] - this.options.from ) * 100 / this.interval ):0;
			var highPrc = this.interval!=0?( ( values[1] - this.options.from ) * 100 / this.interval ):0;
