...
//pre("hello");
        ...
        if ($argArrPost['frmRegion'] > 0) {
            $varWhr = "pkAdminID != " . $argArrPost['frmUserID'] . " AND AdminRegion='" . $argArrPost['frmRegion'] . "' ";
            $arrClms = array('pkAdminID', 'AdminUserName', 'AdminPassword', 'AdminEmail', 'AdminOldPass', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
            if (count($arrCP) > 0) {
                $objCore->setErrorMsg(ADMIN_USE_REGION_ALREADY_TAKEN);
                return false;
            }
        }
        if ($argArrPost['frmCountry'] > 0) {
            $varWhr = "pkAdminID != " . $argArrPost['frmUserID'] . " AND AdminCountry='" . $argArrPost['frmCountry'] . "' AND AdminType = 'user-admin'";
            $arrClms = array('pkAdminID', 'AdminUserName', 'AdminPassword', 'AdminEmail', 'AdminOldPass', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
            if (count($arrCP) > 0) {
                $objCore->setErrorMsg(ADMIN_USE_COUNTRY_ALREADY_TAKEN);
                return false;
            }
        }
        if (!empty($argArrPost['frmPaypalEmail'])) {
            $varWhr = "pkAdminID != " . $argArrPost['frmUserID'] . " AND PaypalEmailID='" . $argArrPost['frmPaypalEmail'] . "' ";
            $arrClms = array('pkAdminID', 'AdminUserName', 'AdminPassword', 'AdminEmail', 'AdminOldPass', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
//            pre($arrCP);
            if (count($arrCP) > 0) {
                $objCore->setErrorMsg("Paypal business email already exist!");
                return false;
            }
        }
        ...
        $varUserWhere = " pkAdminID != " . $argArrPost['frmUserID'] . " AND ( AdminUserName='" . $argArrPost['frmName'] . "' OR AdminEmail='" . $argArrPost['frmUserEmail'] . "' OR PaypalEmailID='" . $argArrPost['frmPaypalEmail'] . "')";
        $arrClms = array('pkAdminID', 'AdminUserName', 'AdminPassword', 'AdminEmail', 'AdminOldPass', 'AdminCountry', 'AdminRegion');
        ...
        if (isset($arrUserList[0]['pkAdminID']) && $arrUserList[0]['pkAdminID'] <> '') {
            ...
            $arrColumnAdd = array(
                ...
                'PaypalEmailID' => trim(stripslashes($argArrPost['frmPaypalEmail'])),
                ...
                'Commission' => trim($argArrPost['frmCommission']),
                'SalesTarget' => trim($argArrPost['frmSalesTarget']),
                'SalesTargetStartDate' => $objCore->serverDateTime($argArrPost['frmSalesTargetStartDate'], DATE_FORMAT_DB),
                'SalesTargetEndDate' => $objCore->serverDateTime($argArrPost['frmSalesTargetEndDate'], DATE_FORMAT_DB),
            );
            ...
            if ($_SERVER[HTTP_HOST] != '192.168.100.97') {
                ...
                $arrBodyKeywordsValues = array(trim(stripslashes($argArrPost['frmName'])), trim(stripslashes($argArrPost['frmName'])), trim($finalPass), trim(stripslashes($argArrPost['frmUserEmail'])), 'Country Portal', SITE_NAME, $varActivationLink, $varPath); //,$varUnsubscribeLink
...
//                die;
            ...
            $objCore->setSuccessMsg(PORTAL_USER_UPDATE_SUCCUSS);
            return true;
        }
    }
...
//        pre($argArrPost);
        ...
        if (isset($arrUserList[0]['pkAdminID']) && $arrUserList[0]['pkAdminID'] <> '') {
            ...
            if ($arrUser[0]['AdminPassword'] == $argArrPost['frmPassword']) {
                ...
                $finalPass = '<br />'.'Password : ' . $argArrPost['frmPassword'];
            }
...
//            pre($arrColumnAdd);
    ...
    function addUser($argArrPost) {
        ...
        if ($argArrPost['frmRegion'] > 0) {
            $varWhr = " AdminRegion='" . $argArrPost['frmRegion'] . "' ";
            $arrClms = array('pkAdminID', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
            if (count($arrCP) > 0) {
                $_SESSION['sessArrUsers'] = $argArrPost;
                $objCore->setErrorMsg(ADMIN_USE_REGION_ALREADY_TAKEN);
        ...
        if ($argArrPost['frmCountry'] > 0) {
            $varWhr = " AdminCountry='" . $argArrPost['frmCountry'] . "' AND AdminType = 'user-admin' ";
            $arrClms = array('pkAdminID', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
            if (count($arrCP) > 0) {
                $_SESSION['sessArrUsers'] = $argArrPost;
        ...
        if (!empty($argArrPost['frmPaypalEmail'])) {
            $varWhr = " PaypalEmailID='" . $argArrPost['frmPaypalEmail'] . "' AND AdminType = 'user-admin'  ";
            $arrClms = array('pkAdminID', 'AdminUserName', 'AdminPassword', 'AdminEmail', 'AdminOldPass', 'AdminCountry', 'AdminRegion');
            $arrCP = $this->select(TABLE_ADMIN, $arrClms, $varWhr);
//            pre($arrCP);
            if (count($arrCP) > 0) {
...
//            
            ...
            $arrColumnAdd = array(
                ...
                'PaypalEmailID' => trim(stripslashes($argArrPost['frmPaypalEmail'])),
                'AdminPassword' => md5(trim($argArrPost['frmPassword'])),
                'fkAdminRollId' => $arrDefaultRollID,
            ...
            $objCore->setSuccessMsg(PORTAL_USER_ADD_SUCCUSS_MSG);
            return true;
        }
    }
