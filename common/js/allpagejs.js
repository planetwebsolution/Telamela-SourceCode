...
function showShippingPortal(e, t, r, frmShippingMultiCountryID) {
    ...
    $("#showShippingPortal").val(e);
    
    
    0 == e || "" == e ? $("#" + r).html('<option value="0">Select Method</option>') : $.post("admin/ajax.php", {
...
function showdetailsofshippingprice(e, t, thisObj, frmShippingMultiCountryID) {
...
//    $("#" + frmShippingMultiCountryID).empty();
//    
//    0 == e || "" == e ? $("#" + r).html('<option value="0">Select Shipping Geteway</option>') : $.post("admin/ajax.php", {
//        action: "showShippingPortal",
//        q: e,
//        selId: t
//    }, function (e) {
...
//    })
	...
	var logistid = $(thisObj).attr('logisticCmpId');
	 $.post("admin/ajax.php", {
       action: "showdetailsofshippingprice",
       shippingmethodid: e,
       countryid: t,
       logistid: logistid,
//       logisticid: e,
//       shippingmethodid: $("#showShippingPortal").val(),
//       countryid: t
   }, function (e) {
	//   alert(e);
//       $("#productRow tr").not(":first").not(":nth-child(2)").remove();
...
$(document).ready(function() {
    ...
    $("body").on("click", ".customer_review_before_login", function() {
        ...
        $.trim(a) <= 1 && ($('<div class="reviewErMessage">Please login as customer to add review.</div>').insertAfter(e), a = parseInt(a) + 1), setTimeout(function() {
            ...
            $("body").on("click", ".customer_review_multiple_product", function() {
        var e = $(this).parent();
        $.trim(a) <= 1 && ($('<div class="reviewErMessage">Multiple reviews on same product are not allowed.</div>').insertAfter(e), a = parseInt(a) + 1), setTimeout(function() {
            $(".reviewErMessage").remove(), a = parseInt(a) - 1
        }, 4e3)
...
//        e.preventDefault();
	 ...
	 $("#frmCurrentWholesalerPassword").on("blur", function() {
            
        var e = $.trim($("#frmCurrentWholesalerPassword").val());
        $.post(SITE_ROOT_URL + "common/ajax/ajax_wholesaler.php", {action: "checkCurrentPassword", pid: e}, function(e) {
            2 == e && ($("#passError span").html("Current password does not match with our record."), $("#frmCurrentCustomerPassword").val("")), 1 == e && $("#passError span").html(""), setTimeout(function() {
                $("#passError span").html("")
            }, 4e3)
        })
