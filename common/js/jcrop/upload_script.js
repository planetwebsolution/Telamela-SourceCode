Cal(function()
    {
        ...
        Cal('.file_upload').live('change', prepareUpload);
        Cal('.file_upload_attr').live('change', prepareUploadAttr);
        
        Cal('.file_upload_multi').live('change', prepareUploadMultiProducts);
        Cal('.file_upload_attr_multi').live('change', prepareUploadAttrMultiProducts);
        ...
        function prepareUpload(event){
            ...
            image.onload = function() {                            
                if (this.height < MIN_PRODUCT_IMAGE_HEIGHT || this.width < MIN_PRODUCT_IMAGE_WIDTH){
                    ...
                    obj.siblings('.responce').html('<div class="req">Please upload image in between ('+MIN_PRODUCT_IMAGE_WIDTH+')px width and ('+MIN_PRODUCT_IMAGE_HEIGHT+')px height!');
                    // obj.focus();
                    obj.val('');
                    return false;
                }
            ...
            image.onerror = function() {
                ...
                obj.siblings('.responce').html('<div class="req">Not a valid file type: '+ chosen.type+'</div>');
                obj.val('');
        ...
        function uploadFiles(event,obj){
            ...
            $.ajax({
                url: 'ajax_uploader.php?action=uploadProductImage&deflt='+deflt,
                type: 'POST',
                data: data,
                cache: false,
                //dataType: 'json',
                ...
                error: function(jqXHR, textStatus, errorThrown){
                    obj.siblings('.responce').html(textStatus);
                // Handle errors here
                //  
        ...
        function uploadFilesAttr(event,obj){
            ...
            $.ajax({
                url: 'ajax_uploader.php?action=uploadProductImageAttr&optid='+optid,
                type: 'POST',
                data: data,
                cache: false,
                //dataType: 'json',
        ...
        function uploadFilesMultiProducts(event,obj){
            var rownum = obj.parent().parent().attr('id');
               
            if(obj.parent().parent().find("input:radio[name=default]").is(':checked')){
                var deflt = 0;
            }else{
                var deflt = 1;
            }  
            ...
            $.ajax({
                url: 'ajax_uploader.php?action=uploadProductImage&deflt='+deflt+'&rowNum='+rownum,
                type: 'POST',
                data: data,
                cache: false,
                //dataType: 'json',
        ...
        function uploadFilesAttrMultiProducts(event,obj){
            ...
            $.ajax({
                url: 'ajax_uploader.php?action=uploadProductImageAttr&optid='+optid+'&rowNum='+rowNum,
                type: 'POST',
                data: data,
                cache: false,
                //dataType: 'json',
