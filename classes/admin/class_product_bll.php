...
class Product extends Database {
    ...
    function productmulcountrydetailAdmin($pid) {
    ...
    function WholesalerShippingGatwaysList($argWhr) {
        ...
        $varWhr = "";
        $varTbl = TABLE_WHOLESALER_TO_SHIPPING_GATEWAY . " INNER JOIN " . TABLE_SHIP_GATEWAYS . " ON (fkShippingGatewaysID=pkShippingGatewaysID AND fkWholesalerID = '" . $argWhr . "' AND ShippingStatus = 1)";
        $arrRes = $this->select($varTbl, $arrClms, $varWhr, $varOrderBy);
        
        if (!$arrRes) {
        	$arrRes = $this->select(TABLE_SHIP_GATEWAYS, $arrClms, "ShippingStatus = 1 ", $varOrderBy);
        }
...
//         }
    ...
    function countryshippingGatewayList($ctid){
    	$argWhr = "AdminCountry='" . $ctid . "' ";
    	$arrClms = array('pkAdminID', 'AdminUserName');
    	$varOrder = "";
    	$arrRes = $this->select(TABLE_ADMIN, $arrClms, $argWhr, $varOrder);
    	$current_country_portal=$arrRes[0]['pkAdminID'];
    	//pre($current_country_portal);
    	$argWhere="fkportalID = " . $current_country_portal;
    	$arrClms = array(
    			'pkShippingGatewaysID',
    			'ShippingTitle',
    			'ShippingType'
    ...
    function countryshippingGatewayList1($argWhere) {
    ...
    function WholesalerListBYcurrentCountry($currentcountryid) {
    	$varPortalFilter = " AND CompanyCountry ='" . $currentcountryid . "' ";
    	$arrClms = array('pkWholesalerID', 'CompanyName');
    	$varWhere = "WholesalerStatus IN ('active','deactive','suspend') " . $varPortalFilter;
    	$varOrderBy = 'CompanyName ASC ';
...
//        }
                ...
                if (isset($arrPost['radio'])) {
                    ...
                    else
                    {
                        if($arrPost['radio'] == 'local'){
                            $arrCols = array(
                            'fkproduct_id' => $arrAddID,
                            'country_id' => $arrPost['frmCountryID'],
                            'producttype' => $arrPost['radio'],
                            );
                        }else{
                            $arrCols = array(
    ...
    function updateProduct($arrPost, $arrFileName, $arrFileNameAttr) {
        ...
        if ($arrAddID > 0) {
            ...
            //$this->delete(tbl_productmulcountry, $varWhereCondition);
            //pre($arrPost);
            if (isset($arrPost['radio'])) {
...
//        $objOrder->addColumn('Action');
    ...
    function customerReviewList($argWhere = '', $argLimit = '') {
        ...
        $varArrayColoum = array('pkReviewID', 'TR.fkCustomerID', 'TR.fkProductID', 'Reviews', 'ApprovedStatus', 
            'ReviewDateAdded', 'pkCustomerID', 'concat(CustomerFirstName," ",CustomerLastName) as csName', 'pkProductID',
            'ProductName', 'pkCategoryId', 'CategoryName','pkRateID');


        $varTable = TABLE_REVIEW . ' AS TR LEFT JOIN ' . TABLE_CUSTOMER . ' AS TC  ON TR.fkCustomerID=TC.pkCustomerID LEFT JOIN '
                . TABLE_PRODUCT . ' AS TP ON TR.fkProductID=TP.pkProductID LEFT JOIN ' 
                . TABLE_CATEGORY . ' as TCAT ON TP.fkCategoryID=TCAT.pkCategoryId LEFT JOIN ' 
                . TABLE_PRODUCT_RATING . ' as RAT ON TR.fkCustomerID = RAT.fkCustomerID AND RAT.fkProductID = TR.fkProductID';
     ...
     */
    function updateReviewStatus($varStatus, $varId,$pendind_status = null,$varRateID = null) {
       // pre($pendind_status);
		global $objGeneral;
        ...
        $varWhrR = 'pkRateID=' . $varRateID;
        $varStatusR = array('RatingApprovedStatus' => $_REQUEST['status']);
        $arrUpdateID = $this->update(TABLE_PRODUCT_RATING, $varStatusR, $varWhrR);
