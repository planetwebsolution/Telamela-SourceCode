...
//pre($arrRes);
    ...
    function CustomerDetails($varCustomerId) {
        $arrClms = array(
            ...
            'CustomerDateAdded',
            'ResAddressLine1', 'ResAddressLine2', 'ResPostalCode', 'ResCountry', 'ResTown', 'ResPhone','ResState','ResCity',
            'CustomerWebsiteVisitCount', 'SameShipping'
        );
        $argWhere = "pkCustomerID='" . $varCustomerId . "' ";
        $arrRes = $this->select(TABLE_CUSTOMER . ' LEFT JOIN tbl_country ON country_id = ShippingCountry OR country_id = 1_ShippingCountry OR country_id = 2_ShippingCountry ', $arrClms, $argWhere);
...
//pre($arrRes);
     ...
     * @return string $arrRes
     */
    function CustomerDetailsWithCountryName($varCustomerId, $runShipID = null) {
        $arrClms = array(
            'CustomerFirstName',
            'CustomerLastName',
            'CustomerEmail',
            ...
            '1_ShippingFirstName',
            '1_ShippingLastName',
            '1_ShippingOrganizationName',
            '1_ShippingAddressLine1',
            '1_ShippingAddressLine2',
            '1_ShippingCountry',
            '1_ShippingState',
            '1_ShippingCity',
            '1_sc.name as 1_ShippingCountryName',
            '1_ShippingPostalCode',
            '1_ShippingPhone',
            '2_ShippingFirstName',
            '2_ShippingLastName',
            '2_ShippingOrganizationName',
            '2_ShippingAddressLine1',
            '2_ShippingAddressLine2',
            '2_ShippingCountry',
            '2_ShippingState',
            '2_ShippingCity',
            '2_sc.name as 2_ShippingCountryName',
            '2_ShippingPostalCode',
            '2_ShippingPhone',
        );
//        if($runShipID == 1){
//            $RunShipCountry = '1_ShippingCountry';
//        } else if($runShipID == 2) {
//            $RunShipCountry = '2_ShippingCountry';
//        } else {
//            $RunShipCountry = 'ShippingCountry';
//        }
        $varTbl = TABLE_CUSTOMER . " LEFT JOIN " . TABLE_COUNTRY . " as bc ON BillingCountry = bc.country_id  LEFT JOIN " . TABLE_COUNTRY . " as sc ON ShippingCountry = sc.country_id LEFT JOIN " . TABLE_COUNTRY . " as 1_sc ON 1_ShippingCountry = 1_sc.country_id LEFT JOIN " . TABLE_COUNTRY . " as 2_sc ON 2_ShippingCountry = 2_sc.country_id";
        $argWhere = "pkCustomerID='" . $varCustomerId . "' ";
     ...
     */
    function CustomerDetailsForShipping($varCustomerId, $runShipID = null) {
        //pre($varCustomerId);
        $varQuery = "SELECT pkCustomerID,concat(CustomerFirstName,' ',CustomerLastName) AS CustomerName, ShippingAddressLine1,ShippingAddressLine2,ShippingCountry,ShippingState,ShippingCity,name,iso_code_2,iso_code_3,zone, ShippingPostalCode,ShippingPhone,
            1_ShippingFirstName,1_ShippingLastName,1_ShippingOrganizationName,1_ShippingAddressLine1,1_ShippingAddressLine2,1_ShippingCountry,1_ShippingState,1_ShippingCity,1_ShippingPostalCode,1_ShippingPhone,
            2_ShippingFirstName,2_ShippingLastName,2_ShippingOrganizationName,2_ShippingAddressLine1,2_ShippingAddressLine2,2_ShippingCountry,2_ShippingState,2_ShippingCity,2_ShippingPostalCode,2_ShippingPhone
        ...
        if ($_SESSION['RunShippingID'] == 1) {
            $arrCustomerDetails[0]['1_ShippingFirstName'] = $arrCustomerDetails[0]['1_ShippingFirstName'];
            $arrCustomerDetails[0]['1_ShippingLastName'] = $arrCustomerDetails[0]['1_ShippingLastName'];
            $arrCustomerDetails[0]['1_ShippingOrganizationName'] = $arrCustomerDetails[0]['1_ShippingOrganizationName'];
            ...
            $arrCustomerDetails[0]['ShippingState'] = $arrCustomerDetails[0]['1_ShippingState'];
            $arrCustomerDetails[0]['ShippingCity'] = $arrCustomerDetails[0]['1_ShippingCity'];
            ...
            $arrCustomerDetails[0]['2_ShippingFirstName'] = $arrCustomerDetails[0]['2_ShippingFirstName'];
            $arrCustomerDetails[0]['2_ShippingLastName'] = $arrCustomerDetails[0]['2_ShippingLastName'];
            $arrCustomerDetails[0]['2_ShippingOrganizationName'] = $arrCustomerDetails[0]['2_ShippingOrganizationName'];
            ...
            $arrCustomerDetails[0]['ShippingState'] = $arrCustomerDetails[0]['2_ShippingState'];
            $arrCustomerDetails[0]['ShippingCity'] = $arrCustomerDetails[0]['2_ShippingCity'];
...
//echo pre($arrPost);
        ...
        if ($arrUserList) {
            ...
            $arrClms = array(
                ...
                'BillingState' => $objCore->stripTags($arrPost['BillingState']),
                'BillingCity' => $objCore->stripTags($arrPost['BillingCity']),
                ...
                'ShippingState' => $objCore->stripTags($arrPost['ShippingState']),
                'ShippingCity' => $objCore->stripTags($arrPost['ShippingCity']),
                ...
                'ResState' => $objCore->stripTags($arrPost['frmResState']),
                'ResCity' => $objCore->stripTags($arrPost['frmResCity']),
...
//
    ...
    function customerResendlink($argArrPost) {
        global $objGeneral;
        $objCore = new Core();

        $customeremail = $argArrPost['frmUserEmail'];
        //$wholsalerpassword = $argArrPost['frmUserpassword'];
        // pre($wholsalerpassword);
        //$arrAddID = $this->insert(TABLE_WHOLESALER, $arrClms);
        $arrClms = array('pkCustomerID,CustomerFirstName,CustomerDateAdded');
        $argWhere = " CustomerEmail = '$customeremail'";
        $arrRes = $this->select(TABLE_CUSTOMER, $arrClms, $argWhere);
        $arrAddID = $arrRes[0]['pkCustomerID'];


        $objTemplate = new EmailTemplate();

        $varUserName = trim(strip_tags($customeremail));
        $varFromUser = SITE_NAME;
        //$varMessage = $arrPost['frmMessage'];
        $vcode = $objGeneral->getEmailVerificationEncode($customeremail . ',' . $arrAddID . ',' . $arrRes[0]['CustomerDateAdded']);
        $ref = (isset($_POST['frmRef'])) ? 'checkout.php' : 'index.php';
        $VerificationUrl = $objCore->getUrl('verify_my_email.php', array('action' => md5('customer'), 'vcode' => $vcode, 'ref' => $ref));

        $varSiteName = SITE_NAME;

        $varWhereTemplate = " EmailTemplateTitle= 'Customer Registrationresendlink' AND EmailTemplateStatus = 'Active' ";
        ...
        $varKeyword = array('{IMAGE_PATH}', '{SITE_NAME}', '{USER_NAME}', '{CUSTOMER}', '{VERIFICATION_URL}', '{VERIFICATION_URL}');

        $varKeywordValues = array($varPathImage, SITE_NAME, $varUserName, ucfirst($arrRes[0]['CustomerFirstName']), $VerificationUrl, $VerificationUrl);
...
//pre($varOutPutValues);
    ...
    function userLoginFromSocialProvider($arrUserList) {
        ...
        //pre($arrCart);
        if (isset($_SESSION['MyCart']) && $_SESSION['MyCart'] != '') { //echo 'hi';die;
            if (count($arrCart) > 0) {
    ...
    function DeleteAddressRun($run_id) {

        $varWhr = "pkCustomerID = '" . $_SESSION['sessUserInfo']['id'] . "'";
//        pre($_SESSION['sessUserInfo']['id']);
        if ($run_id == 0) {
            $run_id = '';
        } else {
            $run_id = $run_id . '_';
        ...
        $arrClmsUpdate = array(
            $run_id . 'ShippingFirstName' => '',
            $run_id . 'ShippingLastName' => '',
            $run_id . 'ShippingOrganizationName' => '',
            $run_id . 'ShippingAddressLine1' => '',
            $run_id . 'ShippingAddressLine2' => '',
            $run_id . 'ShippingCountry' => '',
            $run_id . 'ShippingPostalCode' => '',
            $run_id . 'ShippingPhone' => '',
        );
        //pre($arrClmsUpdate);
        $arrUpdateID = $this->update(TABLE_CUSTOMER, $arrClmsUpdate, $varWhr);
    ...
    function updateRunTimeShippingDetails($arrPost) {
        $objCore = new Core();
        $varWhr = 'pkCustomerID = ' . $_SESSION['sessUserInfo']['id'];

        $AddressNo = ($arrPost['FormNo'] == 0) ? '' : $arrPost['FormNo'] . '_';

        $arrClmsUpdate = array(
            $AddressNo . 'ShippingFirstName' => $arrPost['frmShippingFirstName'],
            $AddressNo . 'ShippingLastName' => $arrPost['frmShippingLastName'],
            $AddressNo . 'ShippingOrganizationName' => $arrPost['frmShippingOrganizationName'],
            $AddressNo . 'ShippingAddressLine1' => $arrPost['frmShippingAddressLine1'],
            $AddressNo . 'ShippingAddressLine2' => $arrPost['frmShippingAddressLine2'],
            $AddressNo . 'ShippingCountry' => $arrPost['frmShippingCountry'],
            $AddressNo . 'ShippingPostalCode' => $arrPost['frmShippingPostalCode'],
            $AddressNo . 'ShippingPhone' => $arrPost['frmShippingPhone'],
        );

        unset($arrPost['runTimeShippingAddress']);
        unset($arrPost['submit']);
        extract($arrPost);
        //pre($arrPost);

        $arrUpdateID = $this->update(TABLE_CUSTOMER, $arrPost, $varWhr);
    }

    function EditRunTimeShippingDetails($arrPost) {
        $objCore = new Core();
        $varWhr = 'pkCustomerID = ' . $_SESSION['sessUserInfo']['id'];
        unset($arrPost['runTimeShippingAddress']);
        unset($arrPost['submit']);
        ...
        if ($arrPost['FormNo'] == 0) {
            ...
            $AddressNo = $arrPost['FormNo'] . '_';
        }
        unset($arrPost['FormNo']);
        unset($arrPost['EditDetailRunTime']);
        //pre($arrPost);

        $arrClmsUpdate = array(
            $AddressNo . 'ShippingFirstName' => $arrPost['ShippingFirstName'],
            $AddressNo . 'ShippingLastName' => $arrPost['ShippingLastName'],
            $AddressNo . 'ShippingOrganizationName' => $arrPost['ShippingOrganizationName'],
            $AddressNo . 'ShippingAddressLine1' => $arrPost['ShippingAddressLine1'],
            $AddressNo . 'ShippingAddressLine2' => $arrPost['ShippingAddressLine2'],
            $AddressNo . 'ShippingCountry' => $arrPost['ShippingCountry'],
            $AddressNo . 'ShippingState' => $arrPost['ShippingState'],
            $AddressNo . 'ShippingCity' => $arrPost['ShippingCity'],
            $AddressNo . 'ShippingPostalCode' => $arrPost['ShippingPostalCode'],
            $AddressNo . 'ShippingPhone' => $arrPost['ShippingPhone'],
        );
...
//print_r($varUserWhere);exit;
        ...
        if (count($arrUserList) > 0) {
            ...
            if ($arrUserList[0]['CustomerStatus'] == "Active" && $arrUserList[0]['IsEmailVerified'] == "1") {
                    ...
                    if (count($arrCart) > 0) {
                        ...
                        if ((isset($_SESSION['MyCart']['Product']) && $_SESSION['MyCart']['Product'] <> '') || count($updateCartData['Product']) > 0) {
                            ...
                            foreach ($updateCartData['Product'] as $key => $val) {
                                ...
                                //echo $key . '===='. $sessionCartProduct[$key];
                                //print_r($val);
                                //if ($sessionCartProduct[$key] == $key) {
                                    //echo $sessionCartProduct[$key][$key."-0"]['qty']. '++++' . $val[$key."-0"]['qty'] ;
                                    $sessionCartProduct[$key][$key."-0"]['qty'] = $val[$key."-0"]['qty'] +  $_SESSION['MyCart']['Product'][$key][$key."-0"]['qty'];
                                //}
                ...
                $arrClms = array('CustomerWebsiteVisitCount' => ($arrUserList[0]['CustomerWebsiteVisitCount'] + 1));
                $this->update(TABLE_CUSTOMER, $arrClms, $varWhr);
                //code for order reward point by date.
                $arrCart2 = $this->select(TABLE_REWARD_POINT, array('fkCustomerID', 'pkRewardPointID', 'Description', 'Points', 'TransactionType', 'RewardEndDate'), "fkCustomerID='" . $arrUserList[0]['pkCustomerID'] . " '" . " And Description = 'Place an order'" . " And TransactionType = 'pending'");
                // pre($arrCart2);
                //$current_date=date('Y-m-d'); 
                // $current_date='2016-01-18'; 
                $current_date = strtotime(date('Y-m-d'), '2016-01-20');
                // $rewardid= array ();
                //pre($arrCart2);
                foreach ($arrCart2 as $key => $rewardValues) {
//                    echo strtotime(date('Y-m-d'),$rewardValues['RewardEndDate']); 
//                    echo '</br>';
                    //pre($rewardValues);
                    //if($current_date > strtotime(date('Y-m-d'),$rewardValues['RewardEndDate']))
                    if (strtotime(date('Y-m-d')) > strtotime($rewardValues['RewardEndDate'])) {

                        // $rewardid[]=$rewardValues['pkRewardPointID'];
                        $rewardid = $rewardValues['pkRewardPointID'];
                        $varWhr = "pkRewardPointID = '" . $rewardid . "'";
                        $arrClms = array(
                            'TransactionType' => 'credit',
                        );
                        $this->update(TABLE_REWARD_POINT, $arrClms, $varWhr);
                        $objGeneral->updateCustomerRewards($rewardid, $rewardValues['fkCustomerID']);
                    }
                }
...
//   $objCore->setErrorMsg(FRONT_CUSTOMER_DEACTIVE_ERROR_MSG);
                if ($arrUserList[0]['IsEmailVerified'] == 0) {
                    ...
                    return EMAIL_NOT_VERIFIED_ERROR_MSGCustomer;
                } else {
...
//        $limit = '0,12';
        ...
        //$query = "SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName,o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,ot.Amount,Status as status FROM " . TABLE_ORDER . " as o INNER JOIN " . TABLE_ORDER_ITEMS . " as oi ON (o.pkOrderID = oi.fkOrderID AND {$varID}) LEFT JOIN " . TABLE_ORDER_TOTAL . " as ot ON (o.pkOrderID = ot.fkOrderID AND ot.Code='total') GROUP BY pkOrderID,Status,fkWholesalerID order by o.pkOrderID desc LIMIT 1,1";
//        $query = "SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName"
//            . ",o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,ot.Amount,Status as status FROM " 
//            . TABLE_ORDER . " as o INNER JOIN " . TABLE_ORDER_ITEMS . " as oi ON "
//            . "(o.pkOrderID = oi.fkOrderID AND {$varID}) LEFT JOIN " . TABLE_ORDER_TOTAL . ""
//                . " as ot ON (o.pkOrderID = ot.fkOrderID AND ot.Code='total') GROUP BY pkOrderID,Status,"
//                . "fkWholesalerID order by o.pkOrderID desc LIMIT 1,1";

        $query = "SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName"
                . ",o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,sum(oi.ItemTotalPrice)Amount,Status as status FROM "
                . TABLE_ORDER . " as o INNER JOIN " . TABLE_ORDER_ITEMS . " as oi ON "
                . "(o.pkOrderID = oi.fkOrderID AND {$varID})  GROUP BY o.pkOrderID order by o.pkOrderID desc {$limit}";

//          $query = "  SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName,
//              o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,sum(oi.ItemTotalPrice)Amount,
//              Status as status FROM
//              tbl_order as o INNER JOIN tbl_order_items as oi ON 
//              (o.pkOrderID = oi.fkOrderID AND o.fkCustomerID = '235' )
//              GROUP BY pkOrderID 
//              order by o.pkOrderID desc LIMIT 0,12";
        //echo $query; die; 
...
//pre($arrRes);
    ...
    /**
     * SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName,o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,ot.Amount,Status as status FROM tbl_order as o INNER JOIN tbl_order_items as oi ON (o.pkOrderID = oi.fkOrderID AND o.fkCustomerID = '235' ) LEFT JOIN tbl_order_total as ot ON (o.pkOrderID = ot.fkOrderID AND ot.Code='total') GROUP BY pkOrderID,Status,fkWholesalerID order by o.pkOrderID desc LIMIT 0,12
     * function getCustomerOrderNum
     *
     * SELECT o.pkOrderID,SubOrderID,o.TransactionID,o.fkCustomerID,o.CustomerFirstName,o.CustomerLastName,o.OrderStatus,o.OrderDateAdded,sum(oi.ItemTotalPrice)Amount,Status as status FROM tbl_order as o INNER JOIN tbl_order_items as oi ON (o.pkOrderID = oi.fkOrderID AND o.fkCustomerID = '235' ) GROUP BY pkOrderID order by o.pkOrderID desc LIMIT 0,12
    ...
    function getOrderProducts($oid) {
        ...
        $arrClms1 = array('pkOrderItemID', 'SubOrderID', 'fkItemID', 'ItemType', 'ItemName', 'ItemImage', 'fkWholesalerID', 'fkOrderID', 'ItemPrice', 'AttributePrice', 'Quantity', 'ItemTotalPrice', 'Status', 'DisputedStatus', 'DiscountPrice', 'ShippingPrice', 'ItemDetails', 'ItemDateAdded');
...
//pre($_SESSION);
    ...
    function getrewieddetail($oid) {
        //$arrSubOrder = $this->select(TABLE_ORDER_ITEMS, array('distinct(SubOrderID)'), "fkOrderID='" . $oid . "'");
        //$arrSubOrder = $this->select(TABLE_REWARD_POINT, array('fkCustomerID','pkRewardPointID', 'Description', 'Points','TransactionType','RewardEndDate'), "fkOrderID='" . $oid . " '". " And Description = 'Place an order'"." And TransactionType = 'pending'");
        $arrSubOrder = $this->select(TABLE_REWARD_POINT, array('fkCustomerID', 'pkRewardPointID', 'Description', 'Points', 'TransactionType', 'RewardEndDate'), "fkOrderID='" . $oid . " '" . " And TransactionType = 'pending'");
...
//        }
    ...
    function updaterewieddetail($rewardid, $fkCustomerID) {
        global $objGeneral;
        $varWhr = "pkRewardPointID = '" . $rewardid . "'";
        $arrClms = array(
            'TransactionType' => 'credit',
        ...
        $this->update(TABLE_REWARD_POINT, $arrClms, $varWhr);
        $objGeneral->updateCustomerRewards($rewardid, $fkCustomerID);
    }
...
//pre($arrRes);
     ...
     * @return string true
     */
    function userLoginToSaveProduct($argArrPost) { //echo 'hi';
//pre($argArrPost);
        $objCore = new Core();
        $shoppingCart = new ShoppingCart();
        $arrUserFlds = array('pkCustomerID', 'CustomerFirstName', 'CustomerScreenName', 'CustomerEmail', 'CustomerPassword', 'BusinessAddress', 'BillingCountry', 'ShippingCountry', 'CustomerStatus', 'IsEmailVerified', 'CustomerWebsiteVisitCount');
...
// $objCore->setErrorMsg(FRON_END_USER_LOGIN_ERROR);
    ...
    /**
     * function addCustomerSupportMobile
     *
     * This function is used display error or message in box.
     *
     * Database Tables used in this function are : tbl_customer, tbl_admin, tbl_support
     ...
     * @return string $arrAddID
     */
    function addCustomerSupportMobile($arrPost) {
//pre($arrPost);
//pre($_SESSION);
        $objCore = new Core();
        global $objGeneral;
        ...
        if ($arrPost['frmToUserType'] == "Admin") {
            ...
            $varWhr = 'pkCustomerID = ' . $arrPost['custId'];
            $arrRes = $this->select(TABLE_CUSTOMER, $arrClms, $varWhr);
            if ($arrRes['BusinessAddress'] == "Billing") {
                $arrClms2 = array(
                    'pkAdminID',
...
//pre($arrClms);
     ...
     * @return string $arrRes
     */
    function getOrderProductsMobile($oid, $cid) {
        /* $arrClms1 = array('c.pkOrderItemID,c.SubOrderID,c.fkItemID', 'ItemType', 'c.ItemName', 'c.ItemImage', 'c.fkWholesalerID', 'c.fkOrderID', 'c.ItemPrice', 'AttributePrice', 'c.Quantity', 'c.ItemTotalPrice', 'c.Status', 'c.DisputedStatus', 'c.DiscountPrice', 'c.ShippingPrice', 'ItemDetails');
          $varTable1 = TABLE_ORDER_ITEMS . ' as c'; */
        $arrClms1 = array('pkOrderItemID', 'SubOrderID', 'fkItemID', 'ItemType', 'ItemName', 'ItemImage', 'fkWholesalerID', 'fkOrderID', 'ItemPrice', 'AttributePrice', 'Quantity', 'ItemTotalPrice', 'Status', 'DisputedStatus', 'DiscountPrice', 'ShippingPrice', 'ItemDetails', 'ItemDateAdded');
        $varTable1 = TABLE_ORDER_ITEMS;
