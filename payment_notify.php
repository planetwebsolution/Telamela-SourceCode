...
//mail('sandeep.sharma@mail.vinove.com','hi',print_r($_POST,1));
//echo "<pre>";
//print_r($_POST);
//die;
...
{
	if(DEBUG == true) {	
	//	mail("amlana.pattanayak@mail.vinove.com","Can't connect to PayPal","Can't connect to PayPal");
...
//mail("amlana.pattanayak@mail.vinove.com","RES","$res");
...
if (strstr($res, "VERIFIED") != '') {
	...
	//mail("amlana.pattanayak@mail.vinove.com","Verified IPN","Verified IPN");
	
	$CustomerID = (int) str_replace('CustomerID-','',base64_decode($_POST['custom']));
	
	$objPage = new OrderProcessPageCtrl();
	if($CustomerID>0) $objPage->pageLoad($CustomerID,$txn_id,$payment_amount);
