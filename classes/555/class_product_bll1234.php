...
class Product extends Database
{
    ...
    function getProductDetails($argProductId)
    {
        ...
        $varQuery = "SELECT pkProductID,fkCategoryID,CategoryHierarchy,ProductName,FinalPrice, DiscountFinalPrice,YoutubeCode,fkPackageId,Quantity,ProductDescription,ProductTerms,HtmlEditor,ProductDateAdded,MetaTitle,MetaKeywords,MetaDescription,ImageName,count(fkAttributeId) as Attribute,pkWholesalerID,CompanyName, AboutCompany, fkTemplateId, SUM(Rating) as numRating, count(fkCustomerID) as numCustomer,fkWholesalerID FROM " . TABLE_PRODUCT . " LEFT JOIN " . TABLE_PRODUCT_IMAGE . " ON pkProductID = fkProductID INNER JOIN " . TABLE_CATEGORY . " ON  (pkCategoryId = fkCategoryID AND CategoryIsDeleted = '0' AND CategoryStatus = '1') INNER JOIN " . TABLE_WHOLESALER . " ON (fkWholesalerID = pkWholesalerID AND WholesalerStatus = 'active') LEFT JOIN " . TABLE_PRODUCT_TO_OPTION . " as pto ON pkProductID = pto.fkProductId
...
//        }
    ...
    {
        $varQuery = "SELECT pck.fkWholesalerID,CompanyName,CompanyEmail,Quantity,ProductName,fkCategoryID,ProductRefNo,pkProductID,pkPackageID,ProductDescription,PackageName,DiscountFinalPrice,FinalPrice,PackagePrice,ProductImage as ImageName, PackageImage,GROUP_CONCAT(AttributeLabel) as AttributeLabel,GROUP_CONCAT(AttributeOptionValue) as OptionTitle,GROUP_CONCAT(optionColorCode) as optionColorCode,GROUP_CONCAT(AttributeOptionImage) as OptionImage,GROUP_CONCAT(OptionExtraPrice) as OptionExtraPrice,GROUP_CONCAT(proOp.fkAttributeId) AS fkAttributeId,GROUP_CONCAT(attrbuteOptionId) AS attrbuteOptionId FROM tbl_package pck inner join tbl_product_to_package pp on pkPackageId=pp.fkPackageId left join tbl_package_product_attribute_option attrOp on (productId=fkProductId and packageId=pp.fkPackageId) left join tbl_attribute on attributeId=pkAttributeID left join tbl_attribute_option on attrbuteOptionId=pkOptionID left join tbl_product_to_option proOp on (productId=proOp.fkProductId and attributeId=proOp.fkAttributeId and attrbuteOptionId=proOp.fkAttributeOptionId) inner join tbl_product tp on pp.fkProductId = pkProductID left join tbl_wholesaler on pck.fkWholesalerID=pkWholesalerID where pkPackageId='" . $argPackageId . "' AND PackageStatus = '1' group by pkProductID";
