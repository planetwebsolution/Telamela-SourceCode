...
//if($_SESSION['sessUserInfo']['id']==232){
//mail('sandeep.sharma@mail.vinove.com','hi',print_r($objPage->arrProductDetails,1));    
//}
//pre($objPage->arrProductDetails);
//pre($objPage->DeactiveWhID);
foreach ($objPage->arrProductDetails as $productdetails) {

    if (in_array($productdetails['pkWholesalerID'], $objPage->DeactiveWhID)) {
        continue;
    }
...
//    pre($objPage->arrProductDetails);
    if ($i > $objPage->lagyPageLimit) {
        ...
        /*if ($_REQUEST['action'] == 'SelectProductCategory') {
//            pre($productdetails);
            
            if ($productdetails['SpecialFinalPrice'] != 0) {
                $dataFieldForCompare = 'SpecialFinalPrice';
            } else if ($productdetails['DiscountFinalPrice'] != 0) {
                $dataFieldForCompare = 'DiscountFinalPrice';
            } else {
                $dataFieldForCompare = 'FinalPrice';
            }
//            echo $dataFieldForCompare;
            if ((floor($productdetails[$dataFieldForCompare]) >= floor($_REQUEST['frmPriceFrom'])) && (ceil($productdetails[$dataFieldForCompare]) <= ceil($_REQUEST['frmPriceTo']))) {
                ...
                <div class="view view-first">
                    <div class="image_new">
                        ...
                        <div class="price_new">
                            <?php
                            if ($productdetails['offerPrice'] > 0 && $productdetails['offerPrice'] != '') {
                                echo $objCore->getFinalPrice($productdetails['FinalPrice'], $productdetails['offerPrice'], $productdetails['SpecialFinalPrice'], 0, 1);
                            } else {
                                echo $objCore->getFinalPrice($productdetails['FinalPrice'], $productdetails['DiscountFinalPrice'], $productdetails['SpecialFinalPrice'], 0, 1);
                            }
